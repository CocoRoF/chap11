...
# custom tools - NEW: Responses API 기반
from src.code_interpreter_new import CodeInterpreterClient
from tools.code_interpreter_new import code_interpreter_tool, set_code_interpreter_client
    ...
    with st.form("my-form", clear_on_submit=True):
        ...
        if submitted and file is not None:
            if not file.name in st.session_state.uploaded_files:
                # Responses API에서는 Container를 사용하므로 파일 ID만 저장
                file_id = st.session_state.code_interpreter_client.upload_file(
                    file.read()
                )
                st.session_state.custom_system_prompt += f"\n업로드한 파일명: {file.name} (File ID: {file_id})\n"
...
def init_page():
    st.set_page_config(page_title="Data Analysis Agent (Responses API)", page_icon="🤗")
    st.header("Data Analysis Agent 🤗 (Responses API)", divider="rainbow")
    st.sidebar.title("Options")

    # 메시지 초기화 / python runtime 초기화
    clear_button = st.sidebar.button("Clear Conversation", key="clear")
    if clear_button or "messages" not in st.session_state:
        welcome_message = "안녕하세요! 데이터 분석 에이전트입니다. CSV 파일을 업로드하고 분석하고 싶은 내용을 입력해주세요 🤗 (Responses API 사용)"
        st.session_state.messages = [{"role": "assistant", "content": welcome_message}]
        # 대화가 리셋될 때 Code Interpreter의 세션도 다시 생성 (Responses API Container 기반)
